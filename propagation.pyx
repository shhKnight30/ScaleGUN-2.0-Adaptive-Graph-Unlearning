...
cdef class InstantGNN:
	...
	def UpdateEdges(self, list pairs, np.ndarray result, num_thread, rmax):
		cdef vector[pair[uint,uint]] cpp_pairs
		cdef uint a, b
		cpp_pairs.reserve(len(pairs))
		for item in pairs:
			a, b = item[0], item[1]
			cpp_pairs.push_back(pair[uint,uint](a, b))
		return self.c_instantgnn.UpdateEdges(cpp_pairs, Map[MatrixXd](result), num_thread, rmax)

	def UpdateStruct(self, list pairs):
		cdef vector[pair[uint,uint]] cpp_pairs
		cdef uint a, b
		cpp_pairs.reserve(len(pairs))
		for item in pairs:
	...
	def UpdateNodes(self,vector[uint] nodes, np.ndarray result,num_thread,rmax):
		return self.c_instantgnn.UpdateNodes(nodes,Map[MatrixXd](result),num_thread,rmax)
	
	def UpdateFeatures(self,vector[uint] nodes, np.ndarray result,num_thread,rmax):
		return self.c_instantgnn.UpdateFeatures(nodes,Map[MatrixXd](result),num_thread,rmax)

	def GetResidueSum(self,np.ndarray res):
