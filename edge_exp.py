...
def main():
    ...
    edge_index_tensor = torch.tensor(edge_index, dtype=torch.long) # tensor copy for affected nodes 
    del edge_index
    gc.collect()
    ...
    for i in range(args.num_batch_removes):
        ...
        if not edges:
            logger.info(f"Ran out of edges to delete at iteration {i}. Stopping.")
            break
            
        # --- [EXPERIMENT 3: Get Affected Nodes Mask BEFORE graph update] ---
        affected_mask = get_affected_nodes(
            edge_index=edge_index_tensor, removed_items=edges, 
            prop_step=args.prop_step, feat_dim=feat_dim, x=feat
        )
        # --- [Log Filtering Ratio] ---
        n_affected = affected_mask.sum().item()
        logger.info(f"Filtering Ratio: {n_affected} nodes affected out of {num_nodes} total nodes.")
